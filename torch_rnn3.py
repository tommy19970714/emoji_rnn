...
def train(input_tensor, target_tensor, encoder, decoder, encoder_optimizer, decoder_optimizer, criterion, max_length=MAX_LENGTH):
    ...
    loss = 0
    print("input_tensor length",len(input_tensor))
    for ei, input_t in enumerate(input_tensor):
        print("input_tensor = ",input_tensor)
        print(type(input_tensor))
        encoder_output, encoder_hidden = encoder(
            input_t, encoder_hidden)
...
def trainIters(encoder, decoder, n_iters, print_every=1000, plot_every=100, learning_rate=0.01):
    ...
    for iter, train_data in enumerate(train_loader):
        input_tensor, target_tensor = train_data
        input_tensor, target_tensor = Variable(input_tensor.long()).to(device), Variable(target_tensor.long()).to(device)
